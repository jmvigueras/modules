...
resource "aws_network_interface" "ni-fgt-port1" {
  ...
  security_groups   = [aws_security_group.nsg-vpc-hub-mgmt.id]
...
resource "aws_network_interface" "ni-fgt-port2" {
  ...
  security_groups   = [aws_security_group.nsg-vpc-hub-public.id]
...
resource "aws_network_interface" "ni-fgt-port3" {
  ...
  security_groups   = [aws_security_group.nsg-vpc-hub-private.id]
...
resource "aws_network_interface" "ni-server-port1" {
  ...
  security_groups   = [aws_security_group.nsg-vpc-hub-servers.id]
