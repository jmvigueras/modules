...
resource "aws_subnet" "subnet-az1-mgmt-ha" {
  ...
  cidr_block        = cidrsubnet(var.vpc-hub_cidr,2,0)
...
resource "aws_subnet" "subnet-az1-public" {
  vpc_id            = aws_vpc.vpc-hub.id
  cidr_block        = cidrsubnet(var.vpc-hub_cidr,2,1)
  availability_zone = var.region["region_az1"]
  tags = var.tags
}
...
resource "aws_subnet" "subnet-az1-private" {
  vpc_id            = aws_vpc.vpc-hub.id
  cidr_block        = cidrsubnet(var.vpc-hub_cidr,2,2)
  availability_zone = var.region["region_az1"]
  tags = var.tags
}
...
resource "aws_subnet" "subnet-az1-servers" {
  vpc_id            = aws_vpc.vpc-hub.id
  cidr_block        = cidrsubnet(var.vpc-hub_cidr,3,7)
  availability_zone = var.region["region_az1"]
  tags = var.tags
}
