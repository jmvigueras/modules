...
data "template_file" "fgt" {
  ...
  vars = {
    ...
    local_bgp_asn        = var.hub_advpn["hub_asn"]
    spoke_bgp_asn        = var.hub_advpn["spoke_asn"]
    local_bgp_id         = cidrhost(var.hub_advpn["advpn_net"],254)
    local_advpn_i-ip1    = cidrhost(var.hub_advpn["advpn_net"],254)
    site_advpn_i-ip1     = cidrhost(var.hub_advpn["advpn_net"],1)
    local_advpn_i-net1   = cidrhost(var.hub_advpn["advpn_net"],0)
    local_advpn_i-mask1  = cidrnetmask(var.hub_advpn["advpn_net"])
    local_servers_cidr   = var.vpc-hub_cidr
