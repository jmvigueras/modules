...
data "template_file" "fgt-active_all-config" {
  ...
  vars = {
    ...
    fgt_sdn-config    = data.template_file.fgt_active-sdn-config.rendered
  }
}
...
data "template_file" "fgt_active-sdn-config" {
  template = templatefile("${path.module}/templates/fgt-sdn.conf", {
    tenant           = var.tenant_id
    subscription     = var.subscription_id
    clientid         = var.client_id
    clientsecret     = var.client_secret
    rsg              = var.resourcegroup_name
    vnet-spoke-rt    = var.vnet-spoke-rt
    ni-private_ip    = cidrhost(var.fgt-subnet_cidrs["private"], 10)
    ni-public_name   = var.fgt-active-ni_names["port3"]
    cluster-pip_name = var.cluster-public-ip_name
  })
}
...
data "template_file" "fgt_bgp-config" {
  template = templatefile("${path.module}/templates/fgt-bgp.conf", {
    ...
    // rs_bgp-asn      = tostring(var.rs_bgp-asn[0]),
