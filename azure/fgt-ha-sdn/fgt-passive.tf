...
data "template_file" "fgt-passive_all-config" {
  ...
  vars = {
    ...
    fgt_sdn-config    = data.template_file.fgt_passive-sdn-config.rendered
  }
}

data "template_file" "fgt_passive-sdn-config" {
  template = templatefile("${path.module}/templates/fgt-sdn.conf", {
    ...
    ni-private_ip    = cidrhost(var.fgt-subnet_cidrs["private"], 11)
    ni-public_name   = var.fgt-passive-ni_names["port3"]
